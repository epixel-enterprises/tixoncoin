...
This document describes various aspects of running `tixoncoin-core` for **system administrators** (but may be useful to a broader audience).

## Introduction

Tixoncoin Core is responsible for communicating directly with and maintaining 
the Tixoncoin peer-to-peer network. For a high-level introduction to Tixoncoin Core, [watch this talk](https://www.youtube.com/watch?v=pt_mm8S9_WU) on the architecture and ledger basics:

[![Introduction to Tixoncoin Core](https://i.ytimg.com/vi/pt_mm8S9_WU/hqdefault.jpg "Introduction to Tixoncoin Core")](https://www.youtube.com/watch?v=pt_mm8S9_WU)

It will also be useful to understand how [data flows](https://www.tixoncoin.org/developers/tixoncoin-core/software/core-data-flow.pdf) and is stored in tixoncoin-core.

## Why run a node?

Run tixoncoin-core if you want to:
* Obtain the most up-to-date and reliable data from the Tixoncoin network
* Generate extended meta data on activity within the Tixoncoin network (change tracking, etc)
* Submit transactions and their confirmations without depending on a third party
* Extended control on which parties to trust in the Tixoncoin network
* Participate in validating the Tixoncoin network


## Building
See [readme](https://github.com/tixoncoin/tixoncoin-core/blob/master/README.md) for build instructions.
We also provide a [docker container](https://github.com/tixoncoin/docker-tixoncoin-core-horizon) for a potentially quicker set up than building from source.

## Package based Installation
If you are using Ubuntu 16.04 LTS we provide the latest stable releases of [tixoncoin-core](https://github.com/tixoncoin/tixoncoin-core) and [tixoncoin-horizon](https://github.com/tixoncoin/go/tree/master/services/horizon) in Debian binary package format.

See [detailed installation instructions](https://github.com/tixoncoin/packages#sdf---packages)

## Configuring
All configuration for tixoncoin-core is done with a TOML file. By default 
tixoncoin-core loads 

`./tixoncoin-core.cfg`

, but you can specify a different file to load on the command line:

`$ tixoncoin-core --conf betterfile.cfg` 

The [example config](https://github.com/tixoncoin/tixoncoin-core/blob/master/docs/tixoncoin-core_example.cfg) describes all the possible 
configuration options.

Here is an [example test network config](https://github.com/tixoncoin/docker-tixoncoin-core-horizon/blob/master/testnet/core/etc/tixoncoin-core.cfg) for connecting to the test network.

Here is an [example public network config](https://github.com/tixoncoin/docs/blob/master/other/tixoncoin-core-validator-example.cfg) for connecting to the public network.

The examples in this file don't specify `--conf betterfile.cfg` for brevity.

## Running
Tixoncoin-core can be run directly from the command line, or through a supervision 
system such as `init`, `upstart`, or `systemd`.

Tixoncoin-core sends logs to standard output and `tixoncoin-core.log` by default, 
...
`$ tixoncoin-core -c "ll?level=debug"`
...
`$ tixoncoin-core -c "ll?level=debug&partition=history"` 
...
Tixoncoin-core can be gracefully exited at any time by delivering `SIGINT` or
...
Tixoncoin-core can also be packaged in a container system such as Docker, so long 
as `BUCKET_DIR_PATH`, `TMP_DIR_PATH`, and the database are stored on persistent 
volumes. For an example, see [docker-tixoncoin-core](https://github.com/tixoncoin/docker-tixoncoin-core-horizon).

Note: `BUCKET_DIR_PATH` and `TMP_DIR_PATH` *must* reside on the same volume
as tixoncoin-core needs to rename files between the two.

## Administrative commands
While running, interaction with tixoncoin-core is done via an administrative 
HTTP endpoint. Commands can be submitted using command-line HTTP tools such 
as `curl`, or by 

`$ tixoncoin-core -c <command>`

. The endpoint is not intended to be exposed to the public internet. It's typically accessed by administrators, 
or by a mid-tier application to submit transactions to the Tixoncoin network. 
...
Currently tixoncoin-core requires very modest hardware. It would be fine to run 
...
See a [list of other validators](https://github.com/tixoncoin/docs/blob/master/validators.md).

## Database
Tixoncoin-core stores the state of the ledger in a SQL database. This DB should 
...
see [example config](https://github.com/tixoncoin/tixoncoin-core/blob/master/docs/tixoncoin-core_example.cfg).

When running tixoncoin-core for the first time, you must initialize the database:

`$ tixoncoin-core --newdb`
...
## Buckets
Tixoncoin-core stores a duplicate copy of the ledger in the form of flat XDR files 
...
can be ignored--they are managed by tixoncoin-core, but they should be stored on 
a fast local disk with sufficient space to store several times the size of the 
current ledger. 

## History archives
Tixoncoin-core normally interacts with one or more "history archives," which are 
...
The [example config](https://github.com/tixoncoin/tixoncoin-core/blob/master/docs/tixoncoin-core_example.cfg) 
shows how to configure a history archive through command templates. 

While it is possible to run a tixoncoin-core node with no configured history 
...
`$ tixoncoin-core --newhist <historyarchive>`
 ...
 * the base reserve used to calculate the tixoncoin balance needed to store things in the ledger
...
## A brief explanation
An important distinction in Tixoncoin compared to traditional quorum based
...
1. `$ tixoncoin-core -c 'upgrades?mode=set&upgradetime=2018-01-31T20:00:00Z&protocolversion=9'`

2. `$ tixoncoin-core -c info`
...
```
   "info" : {
      ...
      "network" : "Public Global Tixoncoin Network ; September 2015",
      "numPeers" : 12,
      "protocol_version" : 1,
      "quorum" : {
         "474366" : {
...
`$ tixoncoin-core --c 'quorum?node=$sdf1` or `$ tixoncoin-core --c 'quorum?node=@GABCDE` 
...
upgrading to a new version of tixoncoin-core that supports a new version of the
...
of this syncing time is simply tixoncoin-core waiting for the next history 
...
* [architecture.md](https://github.com/tixoncoin/tixoncoin-core/blob/master/docs/architecture.md) 
  describes how tixoncoin-core is structured internally, how it is intended to be 
