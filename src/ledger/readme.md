# What is a ledger?
A ledger represents the state of the Tixoncoin universe at a given point in time.
...
[`src/xdr/Tixoncoin-ledger.x`](../xdr/Tixoncoin-ledger.x)
...
tixoncoins present at a given time.
...
During consensus, nodes work together to decide on the value of TixoncoinValue.
TixoncoinValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'TixoncoinValue' should be able to transition to the same new ledger.
...
[`src/xdr/Tixoncoin-ledger-entries.x`](../xdr/Tixoncoin-ledger-entries.x)

### AccountEntry
This entry represents an account. In Tixoncoin, everything is centered around
...
Offers are a way to automate simple trading inside the Tixoncoin network.
