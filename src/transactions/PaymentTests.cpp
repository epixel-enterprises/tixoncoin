...
TEST_CASE("payment", "[tx][payment]")
{
    ...
    SECTION("send TXC to an existing account")
    {
        for_all_versions(*app, [&] {
    ...
    SECTION("send TXC to a new account (no destination)")
    {
        for_all_versions(*app, [&] {
    ...
    SECTION("create, path payment, merge, create")
    {
        ...
        auto tx = sourceAccount.tx({
            ...
            createDestination.op(pathPayment(payDestination, txc, payAmount,
                                             txc, payAmount, {})),
