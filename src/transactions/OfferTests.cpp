...
TEST_CASE("create offer", "[tx][offers]")
{
    ...
    SECTION("create offer errors")
    {
        ...
        SECTION("create offer without TXC to make for reserve")
        {
            auto a1 = root.create("A", minBalance2);
            a1.changeTrust(idr, trustLineLimit);
            a1.changeTrust(usd, trustLineLimit);
                ...
                for (auto const& p : invalidPrices)
                {
                    REQUIRE_THROWS_AS(
                        market.requireChangesWithOffer(
                            {},
                            [&] {
                                return market.addOffer(a, {txc, idr, p, 150});
                            }),
    ...
    SECTION("create offer")
    {
        ...
        SECTION("idr -> txc")
        {
            for_all_versions(*app, [&] {
                market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(a1, {txc, idr, Price{3, 2}, 100});
                });
            });
        }
        ...
        SECTION("txc -> idr")
        {
            ...
            SECTION("crossing + create")
            {
                ...
                auto a1Offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(a1, {idr, txc, Price{1, 1}, a1IDrs});
                });
                ...
                auto checkCrossed = [&](TestAccount& b1, int64 actualPayment,
                    ...
                    auto b1Offer =
                        market.addOffer(b1, {txc, idr, oneone, offerAmount},
                                        OfferState::DELETED);
                    market.requireBalances(
                        {{a1,
                          {{txc, a1Before + actualPayment},
                           {idr, trustLineBalance - actualPayment}}},
                         {b1,
                          {{txc, b1Before - txfee - actualPayment},
                           {idr, actualPayment}}}});
                };
    ...
    SECTION("crossing offers with rounding")
    {
        ...
        auto bidding = OfferState{txc, idr, bidPrice, bidAmount};
        auto asking = OfferState{idr, txc, askPrice, askAmount};
        ...
        SECTION("bid before ask uses bid price")
        {
            ...
            for_versions_to(2, *app, [&] {
                ...
                // 4715278
                // (rounding down)
                auto updatedAsking = OfferState{idr, txc, askPrice, 4715278};
                // rounding error, should be 0
                auto updatedBidding = OfferState{txc, idr, bidPrice, 1};
            ...
            for_versions_from(3, *app, [&] {
                ...
                // 4715278
                // (rounding up)
                auto updatedAsking = OfferState{idr, txc, askPrice, 4715277};
                market.requireChangesWithOffer(
        ...
        SECTION("ask before bid uses ask price")
        {
            for_all_versions(*app, [&] {
                ...
                // 61363625
                auto updatedBidding = OfferState{txc, idr, bidPrice, 61363625};
