...
static std::string
msgSummary(TixoncoinMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(TixoncoinMessage const& msg)
{
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            << "("
...
void
Peer::recvMessage(TixoncoinMessage const& tixoncoinMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(tixoncoinMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (tixoncoinMsg.type() != HELLO) &&
        (tixoncoinMsg.type() != AUTH) && (tixoncoinMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay")
            << "recv: " << tixoncoinMsg.type() << " before completed handshake";
        mDropInRecvMessageUnauthMeter.Mark();
        drop();
        return;
    }

    assert(isAuthenticated() || tixoncoinMsg.type() == HELLO ||
           tixoncoinMsg.type() == AUTH || tixoncoinMsg.type() == ERROR_MSG);

    switch (tixoncoinMsg.type())
    {
    case ERROR_MSG:
    {
        auto t = mRecvErrorTimer.TimeScope();
        recvError(tixoncoinMsg);
    }
    break;
    ...
    case HELLO:
    {
        ...
        this->recvHello(tixoncoinMsg.hello());
    }
    break;
    ...
    case GET_SCP_QUORUMSET:
    {
        ...
        recvGetSCPQuorumSet(tixoncoinMsg);
    }
    break;
...
void
Peer::recvDontHave(TixoncoinMessage const& msg)
{
...
void
Peer::recvGetTxSet(TixoncoinMessage const& msg)
{
    auto self = shared_from_this();
...
void
Peer::recvTxSet(TixoncoinMessage const& msg)
{
...
void
Peer::recvTransaction(TixoncoinMessage const& msg)
{
...
void
Peer::recvGetSCPQuorumSet(TixoncoinMessage const& msg)
{
...
void
Peer::recvSCPQuorumSet(TixoncoinMessage const& msg)
{
...
void
Peer::recvSCPMessage(TixoncoinMessage const& msg)
{
...
void
Peer::recvGetSCPState(TixoncoinMessage const& msg)
{
...
void
Peer::recvError(TixoncoinMessage const& msg)
{
...
void
Peer::recvAuth(TixoncoinMessage const& msg)
{
    if (mState != GOT_HELLO)
    {
...
void
Peer::recvGetPeers(TixoncoinMessage const& msg)
{
    sendPeers();
}

void
Peer::recvPeers(TixoncoinMessage const& msg)
{
