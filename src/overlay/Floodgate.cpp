...
Floodgate::FloodRecord::FloodRecord(TixoncoinMessage const& msg, uint32_t ledger,
                                    Peer::pointer peer)
...
bool
Floodgate::addRecord(TixoncoinMessage const& msg, Peer::pointer peer)
{
    if (mShuttingDown)
    {
        return false;
...
void
Floodgate::broadcast(TixoncoinMessage const& msg, bool force)
{
    if (mShuttingDown)
    {
        return;
