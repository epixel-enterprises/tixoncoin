...
#include "util/asio.h"
#include "TixoncoinCoreVersion.h"
...
static const struct option tixoncoin_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: tixoncoin-core [OPTIONS]\n"
          ...
          "      --forcescp           Next time tixoncoin-core is run, SCP will "
          "start "
          ...
          "signtxn\n"
          "                           (Default is TIXONCOIN_NETWORK_ID "
          "environment\n"
          "variable)\n"
          "      --test               Run self-tests\n"
        ...
        Json::Value& catchupInfo)
{
    ...
    catch (std::invalid_argument const&)
    {
        ...
        LOG(INFO) << "* If you really want to catchup to " << to
                  << " run tixoncoin-core with --newdb parameter.";
        LOG(INFO) << "*";
        return 2;
    }
...
{
    LOG(INFO) << "Starting tixoncoin-core " << TIXONCOIN_CORE_VERSION;
...
main(int argc, char* const* argv)
{
    ...
    std::string cfgFile("tixoncoin-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", tixoncoin_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
        ...
        case OPT_VERSION:
            std::cout << TIXONCOIN_CORE_VERSION << std::endl;
            return 0;
        case OPT_HELP:
        default:
            usage(0);
