...
The `get` and `put` commands define subprocesses that `tixoncoin-core` will
...
called `s3tixoncoin` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3tixoncoin]
get="curl http://history.tixoncoin.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.tixoncoin.org/{1}"
~~~~

In this example, `tixoncoin-core` will use the `curl` command to fetch history and
