...
{
    return std::string("tixoncoin-history.json");
}
...
HistoryArchiveState::HistoryArchiveState() : server(TIXONCOIN_CORE_VERSION)
{
    uint256 u;
...
HistoryArchiveState::HistoryArchiveState(uint32_t ledgerSeq,
                                         BucketList& buckets)
    : server(TIXONCOIN_CORE_VERSION), currentLedger(ledgerSeq)
{
