...
void
HerderSCPDriver::restoreSCPState(uint64_t index, TixoncoinValue const& value)
{
...
HerderSCPDriver::getValueString(Value const& v) const
{
    ...
    try
    {
        ...
        return tixoncoinValueToString(b);
    }
    catch (...)
    {
        return "[:invalid:]";
                                   ...
                                   std::set<Value> const& candidates)
{
    ...
    TixoncoinValue comp(h, 0, emptyUpgradeSteps, 0);
    ...
    std::vector<TixoncoinValue> candidateValues;

    for (auto const& c : candidates)
    {
        candidateValues.emplace_back();
        TixoncoinValue& sv = candidateValues.back();
...
HerderSCPDriver::valueExternalized(uint64_t slotIndex, Value const& value)
{
    ...
    catch (...)
    {
        ...
        // therefore contain a valid TixoncoinValue.
        CLOG(ERROR, "Herder") << "HerderSCPDriver::valueExternalized"
                              << " Externalized TixoncoinValue malformed";
...
void
HerderSCPDriver::nominate(uint64_t slotIndex, TixoncoinValue const& value,
                          TxSetFramePtr proposedSet,
                          TixoncoinValue const& previousValue)
{
