...
void
HerderImpl::valueExternalized(uint64 slotIndex, TixoncoinValue const& value)
{
    updateSCPCounters();
...
HerderImpl::triggerNextLedger(uint32_t ledgerSeqToTrigger)
{
    ...
    TixoncoinValue newProposedValue(txSetHash, nextCloseTime, emptyUpgradeSteps,
                                  0);
    ...
    std::vector<TransactionFramePtr> const& applied)
{
    ...
    // propagation
    {
        ...
        {
            auto msg = tx->toTixoncoinMessage();
