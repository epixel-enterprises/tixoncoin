...
class HerderSCPDriver : public SCPDriver
{
  public:
    struct ConsensusData
    {
        ...
        TixoncoinValue mConsensusValue;
        ConsensusData(uint64_t index, TixoncoinValue const& b)
    ...
    void restoreSCPState(uint64_t index, TixoncoinValue const& value);
    ...
    void nominate(uint64_t slotIndex, TixoncoinValue const& value,
                  TxSetFramePtr proposedSet, TixoncoinValue const& previousValue);
  ...
  private:
    ...
    validateValueHelper(uint64_t slotIndex, TixoncoinValue const& sv) const;
