...
bool force_sqlite = (std::getenv("TIXONCOIN_FORCE_SQLITE") != nullptr);
...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("tixoncoin-core-test");

        std::string rootDir = gTestRoots.back().getName();
        rootDir += "/";

        cfgs[instanceNumber] = tixoncoin::make_unique<Config>();
        Config& thisConfig = *cfgs[instanceNumber];

        std::ostringstream sstream;

        sstream << "tixoncoin" << instanceNumber << ".log";
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Testing tixoncoin-core " << TIXONCOIN_CORE_VERSION;
